...
parser.add_argument('--network', type=str, default="tc_net_mod_5khz_small", help='embeding size (50, 100, 200, 300 only)')
...
parser.add_argument('--save_every', type=int, default=1, help='save state every x iteration')
...
if (args.equal_split):
    train_listfile = open("trainEqual.csv", "r")
    #test_listfile = open("valEqual.csv", "r")
    test_listfile = open("testingData.csv", "r")
else:
    train_listfile = open("trainEqual.csv", "r")
    test_listfile = open("testingData.csv", "r")
...
args_dict['png_folder'] = "/home/kevin/Richi/Spoken-language-identification-master/imagen_train_2/"
...
def do_epoch(mode, epoch):
    ...
    for i in range(0, np.int32(batches_per_epoch)):
    ...
    if (mode == "predict"):
        ...
        with open(pred_filename, 'wb+') as pred_csv:
